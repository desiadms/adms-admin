...
async function dataBase64Enahanced(
	...
	const result: TDateEntryPdf[] = [];

	if (!data) return result;

	// Collect all images from all tasks first.
	// If you want to processes them in batch we can't rely on the task
	// as tasks can have 0-3 images
	const allImages = data.flatMap((task) =>
		task.images.map((img) => ({
			taskId: task.id,
			image: img,
		})),
	...
	for (let i = 0; i < allImages.length; i += 5) {
		...
		// Build a map of processed images
		const processedImages = new Map(
			batchResults.map(({ imageId, base64 }) => [imageId, base64]),
		);

		// Update any tasks that had images in this batch
		for (const task of data) {
			const existingTask = result.find((t) => t.id === task.id);
			if (existingTask) continue;

			const enhancedImages = task.images.map((img) => ({
				...img,
				base64: processedImages.get(img.id) || base64ImagesMap.get(img.id),
			}));

			if (enhancedImages.every((img) => img.base64)) {
				result.push({
					...task,
...
function DownloadButton({ loading }: { loading: boolean }) {
	console.log("im here in here", loading);
	return (
		<>
			{loading ? (
...
function DownloadConfirm({ data }: { data: TDateEntryPdf[] }) {
	return (
		...
		>
			{/* @ts-expect-error - types are not done properly in PDFDownload link */}
